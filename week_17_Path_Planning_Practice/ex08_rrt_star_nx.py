...
class RRTStar(object):
    ...
    def steer(self, node_from, node_to, u=None):
        if u is None:
            u = self.config["eta"]
        # Steer from node_from towards node_to with a step size of u
        vec = node_to - node_from
        dist = np.linalg.norm(vec)
        if dist < u:
            return node_to
        else:
            unit_vec = vec / dist
            new_node = node_from + u * unit_vec
            return new_node
    ...
    def is_collision_free(self, node_from, node_to, obstacles, step=0.2):
        ...
        for i in range(num_steps):
            x = node_from[0] + dx*i/num_steps
            y = node_from[1] + dy*i/num_steps

        for obstacle in obstacles:
            obstacle_dist = np.sqrt((x-obstacle.x)**2 + (y-obstacle.y)**2)
            if obstacle_dist <= obstacle.r:
                col = True
        return col

    # Find adjacent nodes
    def get_near_node_ids(self, new_node, draw):
        r = self.config["gamma_rrt_star"] * np.sqrt(np.log(len(self.G.nodes)) / len(self.G.nodes))
        near_node_ids = []
        for node_id in self.G.nodes:
            node = self.get_node(node_id)
            if np.linalg.norm(node - new_node) < r:
                if draw:
                    plt.plot([node[0], new_node[0]], [node[1], new_node[1]], 'g-', alpha=0.3)
    ...
    def get_distance(self, node_from_id, node_to_id):
        ...
        dist = np.linalg.norm(node_from - node_to)
        return dist
